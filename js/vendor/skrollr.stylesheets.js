...
(function(window, document, undefined) {
		...
		for(var selectorIndex = 0; selectorIndex < selectors.length; selectorIndex++) {
			...
			for(keyframeName in keyframes) {
				for(elementIndex = 0; elementIndex < elements.length; elementIndex++) {
                    ...
                    // Dont append keyframes, just overwrite
    ...
    // Export refresh function so we can apply to dyanmic eleemnts
    var refresh = function() {
        console.log('SKCSS REF')
        kickstart(document.querySelectorAll('link, style'));

    }
    
    window.skrefresh = refresh
}(window, document));
